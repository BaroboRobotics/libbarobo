...
right of the section labeled ``Move'' is clicked, the robot will move its joints
...
Buttons ``A'' and ``B'' pressed together may be used to toggle the robot between holding its zero position 
...
demo programs for controlling Mobot. Ch Student Edition is free for students.
Ch Standard Edition is free for both academic and commercial uses. Ch is
available from SoftIntegration, Inc. at
...
moves joints 1 and 4. Although the programs are presented in Ch, the core
concepts and member functions are applicable to C++ and other language
bindings.
...
The beginning of every robot control program will include header files. Each
...
should remain flexible and back-drivable. \texttt{ROBOT\_HOLD}
...
mobots. Since the the last command sent to \texttt{robot1} was a \texttt{moveNB()}
...
\texttt{robot1} and \texttt{robot2}, to perform an inchworm motion four times, followed
...
and move the robot into its zero position.
\begin{verbatim}
...
\texttt{robotRecordData\_t} & Recorded time and angle data. \\
\hline
\end{tabular}
...
Value & Description \\
\hline
\texttt{ROBOT\_NEUTRAL}& This value indicates that the joint is not moving and is not actuated. The joint is freely back-drivable. \\
...
\texttt{ROBOT\_HOLD}& This value indicates that the joint is currently not moving and is holding its current position. The joint is not currently back-drivable. \\
\hline
\end{tabular}

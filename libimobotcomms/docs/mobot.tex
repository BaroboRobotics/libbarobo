...
motor speeds are set to 50\% speed, and the function \texttt{moveToZero()} is
...
\verbatiminput{../demos/multiple_modules/multipleModules.ch}
...
\texttt{MOBOT\_JOINT3} & Joint number 4 on the MoBot, which is a faceplate joint. 
\end{tabular}
...
\index{MOBOT\_NEUTRAL}
\index{MOBOT\_FORWARD}
\index{MOBOT\_BACKWARD}
\begin{tabular}{p{3cm}p{7cm}} \hline 
Value & Description \\
\hline 
\texttt{MOBOT\_NEUTRAL} & This value indicates automatic direction control for a joint. 
The MoBot will choose the best direction to attain the commanded joint position. \\
\texttt{MOBOT\_FORWARD} & Move the joint in the forward direction. \\
\texttt{MOBOT\_BACKWARD} & Move the joint in the backward direction. \\
\end{tabular}

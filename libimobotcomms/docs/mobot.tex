...
Currently, there are three major flavors of Mobots in use; The Mobot-A, Mobot-I, and
...
``Mobot-A'' will be used to refer specifically to the robot shown in
Figure \ref{fig:joint_diagram_verbose.png}. The term ``Mobot'' will refer all
forms of Mobot, including the Mobot-A, -I, and -L. 
...
\subsection{Introducing the Mobot-A}

\begin{figure}[H]
\begin{center}
\includegraphics[width=4.5in]{images/joint_diagram_verbose.png}
\end{center}
\caption{\label{fig:joint_diagram_verbose.png} A schematic diagram of a Mobot-A module.}
\end{figure}

Figure \ref{fig:joint_diagram_verbose.png} shows a schematic diagram displaying the
locations and positive directions of the four joints of a Mobot-A module. The
...
\section{\label{sec:pairing}Configuring the Mobot-A for Remote Control}
Mobot-A modules should be configured the first time they are used with 
a new computer. The process informs the computer which Mobot-As it
is allowed to connect to. The is also necessary for certain 
functions in the \texttt{CMobot} API, such as \texttt{connect()},
to determine which Mobot-As to connect to.
...
and screenshots showing how to configure your Mobot-As.
...
Mobot-A is turned on, and it should appear as a device in the dialog, as shown in Figure \ref{fig:mac_pairing3}.
If there are multiple Mobot-A devices, as shown in the figure, you may pinpoint the specific Mobot-A to pair
with by looking inside the battery compartment on the side of the Mobot-A with the power switch. Inside,
there is a sticker with the Mobot-A's unique ID number. The last four digits of the ID number will
coincide with the Mobot-A's Bluetooth ID number. Once the correct Mobot-A is selected, click on the ``Continue''
button.
...
At this point, the pairing dialog will attempt to pair with the Mobot-A using a default pairing key, ``0000'', as
shown in Figure \ref{fig:mac_pairing4}. This initial pairing attempt will fail because the Mobot-A pairing key
...
You should be greeted with the dialog shown in Figure \ref{fig:mac_pairing6}, which indicates that the Mobot-A
has successfully paired. At this point, you may continue on with the rest of the pairing process, or 
repeat this step for all Mobot-As that need to be paired before continuing.
...
you to manage, connect, and disconnect from Mobot-As. 

First, add your Mobot-A address(es) into BaroboLink by typing in the address and
...
Each additional Mobot-A added occupies its own row in the dialog. After the addition
...
\end{center}
\caption{\label{fig:shot4_a.png} The Connection dialog two Mobot-A entries.}
\end{figure}
...
the current connection status of the robot. A red dot indicates that the Mobot-A
is not currently connected, and a green dot indicates that the Mobot-A is 
...
\end{center}
\caption{\label{fig:shot4.png} The Connection dialog showing a connected Mobot-A.}
\end{figure}
...
fail, please check that the address has been entered correctly, the Mobot-A is on,
your computer has Bluetooth capability, and that the Mobot-A is within range of 
your computer, and try again.
Note that in Figure \ref{fig:shot4.png}, an additional
Mobot-A had been added to the list of known Mobot-As to illustrate how the dialog
behaves as more Mobot-As are added. Each additional Mobot-A appears as its own
row in the dialog.

Once the connection succeeds, the ``Connect'' button for that Mobot-A turns into
...
a maximum of 7 Mobot-As may be connected to a computer simultaneously, if no
...
maximum number of connected Mobot-As decreases to 5, for a total of 7 connected 
devices.

\subsection{Upgrading the Mobot-A's Firmware}
Upon connecting to Mobot-As, BaroboLink is able to determine if the firmware 
currently residing on the Mobot-A is up to date. If the firmware is not up to 
...
Mobot-A with out-of-date firmware.}
\end{figure}

Upgrading the firmware of a Mobot-A is optional, but Mobot-As with out-of-date firmware
may have missing functionality. It is recommended to update the firmware of
out-of-date Mobot-As as soon as possible. The update process is started by
...
button on the Mobot-A and waits until the button-press is detected. The 
...
may take several minutes, and the Mobot-A should not be turned off or moved away from
...
\subsection{Forcing a Mobot-A's Firmware to Reset}
In the rare event that a Mobot-A's firmware gets corrupted, it is possible to 
reset the firmware to a ``safe'' version. Symptoms of a Mobot-A whose firmware
may have been corrupted include the following:
\begin{itemize}
\item The blue light on the Mobot-A does not turn on.
\item The Mobot-A resets randomly.
...
in \ref{fig:reset1}. To begin the process, type in the address of the Mobot-A to be re-flashed
...
\end{center}
\caption{\label{fig:reset1} The opening dialog for force-reflashing a Mobot-A.}
\end{figure}

The rest of the process is similar to the normal upgrading process of the Mobot-A. Once
the ``safe'' version of the firmware has been flashed onto the Mobot-A, it is recommended to
connect to the Mobot-A using BaroboLink and upgrade the firmware back to the most up-to-date
version.
...
The first time BaroboLink is started, it will try and search for a Mobot-I/L or a Mobot dongle
that is connected using a USB cable. 

There are a couple ways to associate Mobot-I/L modules with BaroboLink. First, BaroboLink
...
Mobot-I/L's may be manually added to BaroboLink by typing the Mobot's serial ID
...
``Connect'' button for each module. Unlike the Mobot-A, there is no maximum limit
...
while connected to a Mobot-I or -L.}
\end{figure}
...
\subsection{Rolling Control (Mobot-A and Mobot-I only)}
...
\subsection{Motions (Mobot-A only)}
...
labeled ``Play''. Note that this functionality only works for the Mobot-A. 
...
Mobot has access to. The Mobot-I and -L robots have an internal accelerometer and
LED color control, which are displayed in the dialog. The Mobot-A has no sensors
so this dialog has no functionality with Mobot-As. 

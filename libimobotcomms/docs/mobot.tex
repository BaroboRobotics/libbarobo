...
To do this, we need to scan the angle data to find out when the joint actually starts moving.
First, we declare a variable that will store the time at which the joint starts moving.
\begin{verbatim}
double startTime;
\end{verbatim}
Next, we begin looping through all of the angle data readings.
We want to find the first angle reading where the absolute value of the angle is greater
than zero. When we find that, we can break out of the loop.
\begin{verbatim}
i = 0;
...
After the loop, the counter variable, 'i', should hold the element index of the first element
where the angle has started changing. Thus, the value of \texttt{time[i]} will hold
the time where the angle starts. We now store that in our variable:
\begin{verbatim}
startTime = time[i];
\end{verbatim}
If we subtract that value from all of our time stamps, we will effectively shift our 
graph to the left such that the first non-zero value for the angle will be on the
y-axis.
\begin{verbatim}
time = time - startTime;
\end{verbatim}

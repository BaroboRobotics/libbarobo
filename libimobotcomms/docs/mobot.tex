...
for(i = 0; i < num; i++) {
    ...
    robot.moveJointTo(ROBOT_JOINT2, 0);      /* Extend joint 2 */
    robot.moveJointTo(ROBOT_JOINT3, 0);      /* Extend joint 3 */
}
\end{verbatim}
...
on the Mobot itself. These timestamps are stored in the \texttt{time1} array and
...
generate, and the computational array \texttt{angles1\_unwrapped} will hold
...
Before we begin the motion, we move the robot into its zero postition.
\begin{verbatim}
robot.moveToZero();
\end{verbatim}
...
Now that all of the data has been acquired, we unwrap and plot the data.
\begin{verbatim}
unwrapdeg(angles1_unwrapped, angles1);
...
\caption{\label{fig:dataacq2_fig1} The unwrapped data recorded by \texttt{dataAcquisition2.ch}.}
\end{figure}
...
Finally, we want to unwrap the data and convert the angle readings to a distance. 
...
be seen that the robot traveled to a distance of 12 inches, and then stopped. 

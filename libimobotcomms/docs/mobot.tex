...
The Mobot is a breakthrough modular mobot. A single Mobot module is a fully 
functional mobot capable of performing many possible motions. The Mobot
can als obe used as a building block to create mobots with different
...
The configuration is performed through the Barobo MobotController
...
Windows users may skip these steps directly to section \ref{sec:pairing_mobotcontroller}
on page \pageref{sec:pairing_mobotcontroller}.
...
\subsection{\label{sec:pairing_mobotcontroller} Adding Bluetooth Addresses of Mobots in MobotController.}
First, start the provided Barobo MobotController application.
In Windows, start the provided Barobo Mobot Control Program by clicking on the icon labeled 
``MobotController'' on your desktop, as shown in \ref{fig:barobo_icon.png}. On Mac OS X systems,
the MobotController application is located inside the ``Applications'' folder in Finder. The 
control dialog as shown in Figure \ref{fig:shot1.png} should pop up.
\begin{figure}[H]
\begin{center}
\includegraphics[width=1in]{images/barobo_icon.png}
\end{center}
\caption{\label{fig:barobo_icon.png} The icon for the Barobo MobotController.}
\end{figure}
...
\end{center}
\caption{\label{fig:shot1.png} The graphical user interface of the MobotController.}
\end{figure}

Click on the menu item ``Configure $\rightarrow$ Configure Mobot Bluetooth'', as
...
\end{center}
\caption{\label{fig:shot3.png} Configuring mobot bluetooth connection.}
\end{figure}

This should bring up a second dialog, titled ``Configure Mobot Bluetooth'', as
...
This dialog allows us to add mobot bluetooth addresses to the list of currently
known mobot bluetooth addresses. To add an address, first type in the address
in the text box on the top of the dialog, as shown in Figure \ref{fig:shot5.png}.
You can find the bluetooth address of each mobot inside the battery compartment
of the mobot on the same side as the power switch. 

\begin{figure}[H]
\begin{center}
\includegraphics[width=3in]{images/shot5.png}
\end{center}
\caption{\label{fig:shot5.png} Adding the mobot bluetooth address in the dialog window.}
\end{figure}
...
address of one of our mobots, which is \texttt{"00:06:66:43:0E:02"}.
...
We use the same process to add our remaining two mobots to the list,
...
\end{center}
\caption{\label{fig:shot8.png} Displaying bluetooth addresses for three mobots.}
\end{figure}

The dialog also allows users to reorder the addresses listed. The
order the addresses are listed in affects the order in which the mobots
...
\end{center}
\caption{\label{fig:shot10.png} Modifying the order of bluetooth addresses with the ``Configure Mobot Bluetooth'' dialog.}
\end{figure}

\subsection{Connecting and Disconnecting to Mobots from the MobotController}
Once bluetooth addresses are added to the MobotController, you may connect to the 
first address by clicking on the ``Connect $\rightarrow$ Connect to Mobot''
menu item. The connected mobot may be disconnected by clicking on the 
``Connect $\rightarrow$ Disconnect from Mobot'' menu item. Any connected mobots are
automatically disconnected upon exiting the program. Note that in order to run a Ch
program that controls a mobot, the mobot should not currently be connected to any
other application, including the MobotController, other Ch programs, and other
...
\end{center}
\caption{\label{fig:shot1_populated.png} The graphical display of the MobotController
while connected to a mobot.}
\end{figure}

Once a mobot is connected to the MobotController, the joint angles and speeds
of the mobot are displayed as shown in Figure \ref{fig:shot1_populated.png}.
The MobotController can then be
used to display
...
A mobot has two 3.5 inch diameter wheels. It is moving at a constant velocity
by turning its wheels 45 degrees per second. 
\begin{enumerate}
\item How long will it take for the mobot to rotate its wheels 2 full rotations? (720 degrees)
\item During that time, how far should the mobot travel? Verify your results by measuring
the distance the real mobot travels during the calculated time.
\end{enumerate}

\subsubsection{Problem Solution}
\begin{enumerate}
\item We are given that a mobot is turning its wheels at a constant speed of 45 degrees per second, and 
...
\begin{equation*}
c = 2 \pi r
\end{equation*} where $r$ is the radius of the circle. Since our mobot will
turn its wheels two full rotations during the motion, it will travel 2 circumferences
in distance. Thus, the distance the mobot should travel is
\begin{equation*}
...
\end{equation*}
Our mobot should travel approximately 22 inches during the 16 second motion.
\end{enumerate}

To verify these results with a real mobot the MobotController program may be used.
\begin{enumerate}
...
the bottom left of the MobotController and then clicking the "Set" button.
...
the mobot forward. The motion may be timed with a stopwatch.
\end{enumerate}

\section{Getting Started with Programming the Mobot}
The MobotController can be used to control the Mobot for simple tasks and applications.
...
Before the Ch program is executed, the Bluetooth addresses of the mobots
need to be added using the MobotController as described in Section \ref{sec:pairing}.
If a mobot is already connected to the MobotController, disconnect
from the mobot before running the Ch program, or close the MobotController application.
...
Alternatively, the demos may be accessed in the MobotController application
...
\begin{figure}
  ...
  \caption{The ``Help $\rightarrow$ Demos'' menu item on the MobotController Application.}
...
\end{verbatim}
use the \texttt{setTwoWheelMobotSpeed()} function to set the faceplate joint
speeds for a mobot acting as a two wheeled car. The
\texttt{setTwoWheelMobotSpeed()} function takes a desired speed
...
Next, we connect our four mobot variables to the first four mobots 
configured in our MobotController.
\begin{verbatim}
mobot1.connect();
mobot2.connect();
mobot3.connect();

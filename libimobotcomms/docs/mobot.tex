...
\begin{enumerate}
\item How long will it take for the robot to rotate its wheels 2 full rotations, or 720 degrees?
...
\begin{enumerate}
\item We are given that a robot is turning its wheels at 45 degrees per second, and 
...
\item While holding the robot off the ground, make the robot begin turning its wheels
by clicking on the up arrows for joints 1 and 4 in the ``Individual Joint Control"
section of the RobotController.
\item Using a stopwatch, place the robot on the ground and time its progress
for 16 seconds. The distance the robot travels should be approximately 22 inches.
\end{enumerate}

Alternatively, the following method may be used:
\begin{enumerate}
\item Set the robot on the ground.
\item Set the joint speeds of joints 1 and 4 to 45 degrees per second.
\item Click on the ``Up'' arrow in the ``Rolling Control'' section of the
Robot Controller. This will roll the robot forward by rotating the faceplates
360 degrees.
\item Once the robot has stopped moving, click again on the ``Up'' arrow. This
will roll the robot an additional 360 degrees, thereby rolling each of the wheels
a total of 720 degrees.
\item The distance the robot has traveled can then be measured, and should be
...
line of code, joints one and four 
move forward while joints two and three hold their current positions. The
...
\begin{figure}
  ...
  \subfloat[End]{\label{fig:inchworm5}\includegraphics[width=1in]{images/inchworm1.jpg}}
...
\end{verbatim}
The first line turns joint 1 360 degrees. However, since it is moved with a non-blocking function,
...
effectively move joints two and three simultaneously. Since these are both
...
A for loop is used to to tumble ``n'' times, as shown in the following code:
\begin{verbatim}
int n = 2;
int i;
for(i = 0; i < n; i++) {
\end{verbatim}
These lines create two new variables, ``n'' and ``i'', which hold the number
of times to tumble, and act as the loop counter variable, respectively. 
...
\end{verbatim}
The entire motion, consisting of two flips, are performed ``n'' times. After
...
\subsubsection{Problem Statement}
Roll a robot forward by rotating the wheels 720 degrees at 45 degrees per
second. Record the motion of joint 1 during the motion and display a plot
of the joint angle versus time.
...
It is possible to recalibrate the Mobot's zero positions. To recalibrate
